...
## move the airfiles with the correct number from the temp ftp folder and print descrep to stdout
...
## move the bsp airfiles with the correct number from the temp ftp folder and print descrep to stdout
#
#sourceDir="/data/stella/.bspfiles-ftp-tmp"
#destDir="/data/stella/bspfiles"
#
#prepString="spec"
#digiLen=5
#fileExt=".AIR"
#
#i=$(cat "/home/dw/.count-bsp")
#ls -a $sourceDir | tail -n +3 | while read fileName; do
#	newFileName=$(printf "$prepString%$(($digiLen-${#i}))s$i$fileExt" | sed 's/ /0/g')
#	((i++))
#	mv "$sourceDir/$fileName" "$destDir/$newFileName"
#	if [ "$?" -eq "0" ]; then
#		echo $i > "/home/dw/.count-bsp"
#	fi
#done
#
