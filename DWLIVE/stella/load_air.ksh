...
if [[ "${step_no}" -ge "${START_STEP_NO}" ]] then
  ...
  then
    ...
    do
      echo ${users}
     mailx -v -s "Details of STELLA AIR (Flexi)  errors encountered `hostname`  on "$today_d" " ${users} < ${appdir}/air_error.err
    done
...
if [[ "${step_no}" -ge "${START_STEP_NO}" ]] then
  ...
  do
    echo ${users}
    mailx -v -s "STELLA AIR  (Flexi) load `hostname` on "$today_d" " ${users} < ${appdir}/air_mail.log
  done
else
  report "Step ${step_no} bypassed.\n"
fi
