...
sqlload $USER/$PASSD  "$iacs_path"act_accom.ctl log="$zipfile_path/logdata/act_accom_$_DATESTMP.log" bad="$zipfile_path/baddata/act_accom_$_DATESTMP.bad">> $f_log_path/iacsactuals$today_d.log errors=1000000
...
echo " ---------------- " >>  $f_log_path/iacsactuals$today_d.log
sqlload $USER/$PASSD "$iacs_path"act_inv.ctl log="$zipfile_path/logdata/act_inv_$_DATESTMP.log"  bad="$zipfile_path/baddata/act_inv_$_DATESTMP.bad">> $f_log_path/iacsactuals$today_d.log errors=1000000
...
echo " -----------------------------"  >>  $f_log_path/iacsactuals$today_d.log
sqlload $USER/$PASSD  "$iacs_path"act_accom_inv.ctl log="$zipfile_path/logdata/act_accom_inv_$_DATESTMP.log" bad="$zipfile_path/baddata/act_accom_inv_$_DATESTMP.bad" >> $f_log_path/iacsactuals$today_d.log errors=1000000
...
echo " ---------------------------"  >>  $f_log_path/iacsactuals$today_d.log
sqlload $USER/$PASSD "$iacs_path"act_extra.ctl log="$zipfile_path/logdata/act_extra_$_DATESTMP.log" bad="$zipfile_path/baddata/act_extra_$_DATESTMP.bad" >> $f_log_path/iacsactuals$today_d.log errors=1000000
...
echo " ----------------------"  >>  $f_log_path/iacsactuals$today_d.log
sqlload $USER/$PASSD "$iacs_path"act_invextra.ctl log="$zipfile_path/logdata/act_invextra_$_DATESTMP.log" bad="$zipfile_path/baddata/act_invextra_$_DATESTMP.bad" >> $f_log_path/iacsactuals$today_d.log errors=1000000
...
#     This bit is added to automate mail from shell script  :
   
#      if there is a bad file 
      
#      OR

#      if there is any records in loader_msgs table
...
select count(*)  var1  from iacs.loader_msgs where to_char(loaded_datetime,'dd-mon-yyyy') = to_char(sysdate,'dd-mon-yyyy');
exit var1
!
temp=$?
...
if [ -s $iacs_path/iacs_error.err ]
 then
echo "error found" 
mailx -s "Details of IACS feedback/loader messages errors encountered  on "$today_d" " basds@firstchoice.co.uk <  $iacs_path/iacs_error.err
...
then
   ...
   echo $mail_txt
   echo $mail_txt 2>&1 |  mailx -s "IACS Feedback errors" basds@firstchoice.co.uk
fi
