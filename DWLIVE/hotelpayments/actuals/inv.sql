...
-- 27/07/07 Paul Butler  Set company code according to aptos_n_code (previous method no longer works now that IACS H&J
--                       invoices are fed back into DW
...
BEGIN
 ...
 BEGIN 
     ...
     VALUES (c_inv.inv_seq_no, c_inv.invoice_num, c_inv.property_num, 
             CASE WHEN c_inv.property_num > 999999999 THEN 'HAJ' WHEN length(c_inv.aptos_n_code) > 18 THEN 'HAJ' ELSE 'FCHF' END,
