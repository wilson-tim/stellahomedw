...
-- 1.17 Leigh           change for iacs v30 cyprus payment office
-- 1.18 Jyoti		Added new column ski_booking_ind in Booking table , for that added table join to otop_product to get brand
-- 1.19 Jyoti         For H & J bookings  update those  accrual's where currency is corrected from ??? to USD 
...
SELECT /*+ INDEX(bfd BASFD_PK)  INDEX(booking BKING_PK)*/ 
        ...
        decode(otop_product.brand_code,'K','Y','N') ski_booking_ind  -- added by JR iacs Ski changes
...
FROM
                    ...
                    V_BASE_FINANCIAL_DETAIL bfd, 
                        ...
                        otop_product  -- added by JR iacs Ski changes
...
--  JR , added for Iacs SKi
and booking.season_year = otop_product.season_year 
and booking.season_type = otop_product.season_type
and booking.otop_product_code = otop_product.otop_product_code
...
SELECT 
        ...
        decode(otop_product.brand_code,'K','Y','N') ski_booking_ind  -- JR , Iacs Ski

FROM

        booking,     /* this is the dataw owned booking table */
        
        otop_product  -- added by JR iacs Ski
...
FOR seas_cur in season_to_extract LOOP
        ...
        FOR c1 in main_cursor(v_season_year, v_season_type) LOOP
                ...
                FOR acc IN accom_sale_cursor(v_season_year, v_season_type) LOOP -- for each accom loop
						...
						ELSIF v_otop_country_code in ('GR','IR','MT','IT','KENY','TN','ASIN','GM','EGYP','BG','IT') THEN
							-- a country paid by Cyprus overseas branch
                                                        v_property_num := 999996;
                                                ELSE 
                        ...
                        IF v_acc_loop = 1 THEN
                                ...
                                INSERT INTO iacs.booking 
                                        ...
                                        ski_booking_ind )  -- new column for SKI bookings
                                ...
                                EXCEPTION
                                        ...
                                        v_statement :=115;
                                          ...
                                          SET arrival_date = c1.hol_departure_date,
                                              ...
                                              ski_booking_ind = c1.ski_booking_ind -- added by JR, iacs Ski changes
	...
	Where b.booking_seq_num = ba.booking_seq_num and
	      ...
	      exists (Select null 
		      ...
		      Where boa.booking_seq_num = ba.booking_seq_num and
			    boa.booking_accom_seq_num <> ba.booking_accom_seq_num and
