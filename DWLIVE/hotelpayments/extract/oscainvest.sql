...
declare
        ...
        v_datecutoffstart DATE := '31-AUG-2000'; -- GREATER THAN THIS DATE
        v_datecutoffend DATE := '1-OCT-2000'; -- LESS THAN THIS DATE
...
SELECT b.booking_seq_num, substr(b.season,1,1) season_type, substr(b.season,2,2) season_year,
        ...
        bai.payment_amt, ba.accrual_ccy_code, aptos_Extracted_date, bai.matched_batch_num batch_num 
...
and b.season = 'S00' and ba.accrual_ccy_code in ('USD','???')
...
FOR cloop IN maincur LOOP
          ...
          exception
                when no_data_found THEN
                        dbms_output.put_line('nodatafound:'||cloop.booking_ref||cloop.accrual_ccy_code||
                           ' :'||cloop.booking_seq_num||':'||cloop.payment_amt||':'||cloop.aptos_extracted_date||
			':'||cloop.batch_num);
                        v_ex_rate :=1;
      end;
        ...
        v_data_rec := (cloop.season_type||','||
                        ...
                        v_payment_uk_amt||','||
			cloop.batch_num||','||cloop.aptos_Extracted_date
                        );
