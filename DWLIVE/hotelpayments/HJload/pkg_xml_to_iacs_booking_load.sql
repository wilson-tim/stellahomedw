...
--  1.2          LA - (16/10/02)    - Add new functionality for currency adjustment
...
v_ccy_adjusted   iacs.booking_Accom.accrual_ccy_code%TYPE;
...
-- Download into iacs sql anywhere database fails if currency code is null so default it to ???
        ...
        -- some currencies need translating to the value we want to use for them
        
        BEGIN
        SELECT target_ccy_code INTO v_ccy_adjusted 
        FROM iacs.currency_translation c
        WHERE c.source_ccy_code = c_bookingaccom_row.accrual_ccy_code;
        
        EXCEPTION
        WHEN no_data_found THEN
        
             v_ccy_adjusted := c_bookingaccom_row.accrual_ccy_code;
        END;
        
        dbms_output.put_line('src:'||c_bookingaccom_row.accrual_ccy_code||',adjccy:'||v_ccy_adjusted);
        ...
        Update iacs.booking_accom
                ...
                Where booking_seq_num      = v_booking_seq_num and
                    ...
                    accrual_ccy_code       = v_ccy_adjusted ;
...
f_log(null);
        ...
        -- Log into iacs_general_error table
           ...
           v_data_record        := substr('Booking Ref/Season:'||c_bookingaccom_row.booking_ref||'/'||c_bookingaccom_row.season||
